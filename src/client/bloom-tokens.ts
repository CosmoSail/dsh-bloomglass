 ...
 */
import { PALETTE, mix, type PaletteEntry, type Variant } from './palette.ts'
import { OTHER_VARIANTS } from './palette.ts'
 ...
 */
export function bloomTokens(p: PaletteEntry, dark: boolean): string {
 ...
 */
export function borderStack(p: PaletteEntry, dark: boolean): string {
  const m = p.morandi
  const tx = dark ? p.txD : p.txL
  return dark
 ...
 */
export function labelStack(p: PaletteEntry, dark: boolean): string {
  const tx = dark ? p.txD : p.txL
  return dark
 ...
 * 就变成绿色以外的东西。
 */
export function sharedDswTokens(p: PaletteEntry, dark: boolean): string {
   ...
   */
  const t = (c: string, pct: number): string => `color-mix(in oklch, ${c}, ${bg} ${pct}%)`
  return `
...
export function mistLight(p: PaletteEntry): string {
...
export function mistDark(p: PaletteEntry): string {
 ...
 */
export function variantBlock(v: Variant, dark: boolean): string {
  const p = PALETTE[v]
...
/** 生成 4 变体 + 亮/暗的完整 CSS。 */
export function buildBloomCSS(): string {
